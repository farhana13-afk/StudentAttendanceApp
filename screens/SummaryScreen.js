...
class SummaryScreen extends React.Component{
 constructor() {
  ...
  render() {
    return (
      <View style={{ flex: 1 , backgroundColor:'#e3e3e3', borderColor: '#d527b7',borderWidth: 1, padding:20}}>
        <View style={{ flex: 0.1 }}>
        ...
        <View style={styles.presentContainer}>
          {
            this.state.present_students.map((student, index)=>(
                <Text style={{fontSize:18, color:'green'}}>{student.name}</Text>
              )
            )
          }
        </View>
        ...
        <View style={styles.absentContainer}>
          {
            this.state.absent_students.map((student, index)=>(
                <Text style={{fontSize:18, color: 'red'}}>{student.name}</Text>
              )
            )
          }
        </View>
        <View style={{flex:0.1, flexDirection:'row', justifyContent:'space-around', marginTop:10, backgroundColor: 'white'}}>
